...
function template_list_themes()
{
	...
	foreach ($context['themes'] as $theme)
	{
		$themeHref = $scripturl . '?action=admin;area=theme;th=' . $theme['id'] . ';' . $context['session_var'] . '=' . $context['session_id'] . ';sa=settings';
		echo '
			<div class="lff-common-section-title">
				<h3>
					<a href="', $themeHref, '">', $theme['name'], '</a>', !empty($theme['version']) ? ' <em>(' . $theme['version'] . ')</em>' : '';
			echo '
				</h3>';

			// You *cannot* delete the default theme. It's important!
			if ($theme['id'] != 1) {
				$themeDeleteHref = $scripturl . '?action=admin;area=theme;sa=remove;th=' . $theme['id'] . ';' . $context['session_var'] . '=' . $context['session_id'];
				echo '<a href="', $themeDeleteHref, '" onclick="return confirm(\'', $txt['theme_remove_confirm'], '\');">';
				echo '<img src="', $settings['images_url'], '/icons/delete.gif" alt="', $txt['theme_remove'], '" title="', $txt['theme_remove'], '" />';
				echo '</a>';
			}
