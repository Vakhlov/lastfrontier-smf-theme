...
function template_main()
{
	...
	while ($message = $context['get_message']())
	{
		...
		if (!$message['member']['is_guest'])
		{
			...
			if ($message['member']['thanks'] > 0)
				echo '
								<li class="thankscount">', $txt['saythanks_thankscount'], ': ', $message['member']['thanks'], ' ', $txt['saythanks_times'] ,'</li>';
			/* Say Thanks mod end */
		...
		// Say Thanks button
		global $user_info;
		if ($user_info['is_guest'] == false && $message['member']['id'] && array_key_exists('message_thanks', $context) && $user_info['id'] != $message['member']['id'] && ( !array_key_exists($message['id'], $context['message_thanks']) || (array_key_exists($message['id'], $context['message_thanks']) && !in_array($user_info['id'], $context['message_thanks'][$message['id']]['users']))))
			echo '
									<li class="thank_you_button"><a href="', $scripturl, '?action=thank;msg=', $message['id'], ';topic=', $context['current_topic'].';member=', $message['member']['id'].'">', $txt['saythanks_text'], '</a></li>';
		...
		if (isset($context['message_thanks'][$message['id']]))
			echo '
						<div class="thanks smalltext"><span>', $txt['saythanks_thanked'], '</span>', $context['message_thanks'][$message['id']]['list'], '</div>';
		/* Say Thanks mod end */
