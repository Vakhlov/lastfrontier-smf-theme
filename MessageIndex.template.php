<?php
/**
 * Возвращает кнопки, которые необзодимо вывести сверху и снизу списка тем.
 */
function getButtons () {
	...
	if (!$context['user']['is_logged'] || !$settings['show_mark_read']) {
		unset($buttons['markread']);
	}

	// Allow adding new buttons easily.
	call_integration_hook('integrate_messageindex_buttons', array(&$buttons));
 ...
 */
function renderListNavigation ($buttons, $side = 'top', $options = []) {
	echo '<div class="lff-list-navigation">';
...
/**
 * Выводит пояснения к используемым иконкам.
 */
function renderLegend () {
	global $context, $modSettings, $settings, $txt;

	if (!$context['no_topic_listing']) {
		echo '<ul class="lff-topiclist-legend">';

		if (!empty($modSettings['enableParticipation']) && $context['user']['is_logged']) {
			echo '<li><img src="' . $settings['images_url'] . '/topic/my_normal_post.gif" alt="" align="middle" /> ' . $txt['participation_caption'] . '</li>';
		}

		echo '<li><img src="' . $settings['images_url'] . '/topic/normal_post.gif" alt="" align="middle" /> ' . $txt['normal_topic'] . '</li>';
		echo '<li><img src="' . $settings['images_url'] . '/topic/hot_post.gif" alt="" align="middle" /> ' . sprintf($txt['hot_topics'], $modSettings['hotTopicPosts']) . '</li>';
		echo '<li><img src="' . $settings['images_url'] . '/topic/veryhot_post.gif" alt="" align="middle" /> ' . sprintf($txt['very_hot_topics'], $modSettings['hotTopicVeryPosts']) . '</li>';
		echo '<li><img src="' . $settings['images_url'] . '/icons/quick_lock.gif" alt="" align="middle" /> ' . $txt['locked_topic'] . '</li>';

		if ($modSettings['enableStickyTopics'] == '1') {
			echo '<li><img src="' . $settings['images_url'] . '/icons/quick_sticky.gif" alt="" align="middle" /> ' . $txt['sticky_topic'] . '</li>';
		}

		if ($modSettings['pollMode'] == '1') {
			echo '<li><img src="' . $settings['images_url'] . '/topic/normal_poll.gif" alt="" align="middle" /> ' . $txt['poll']  . '</li>';
		}
...
/**
 * Выводит форму быстрого перехода.
 */
function renderNavigationForm () {
	global $context, $txt;

	echo '<div class="lff-jump-form"><div id="message_index_jump_to"></div></div>';
...
/**
 * Выводит блок под списком тем.
 */
function renderBottomSection () {

	echo '<div class="lff-topiclist-bottom-section">';
...
function template_main()
{
	...
	if (!$context['no_topic_listing']) {
		renderListNavigation($buttons, 'top');
	...
	<div class="tborder topic_table" id="messageindex">
		...
		{
			echo '
					<th scope="col" class="first_th" width="8%" colspan="2"><div class="lff-topic-list-header-content">&nbsp;</div></th>
					<th scope="col" class="lefttext"><div class="lff-topic-list-header-content"><a href="', $scripturl, '?board=', $context['current_board'], '.', $context['start'], ';sort=subject', $context['sort_by'] == 'subject' && $context['sort_direction'] == 'up' ? ';desc' : '', '">', $txt['subject'], $context['sort_by'] == 'subject' ? ' <img src="' . $settings['images_url'] . '/sort_' . $context['sort_direction'] . '.gif" alt="" />' : '', '</a> / <a href="', $scripturl, '?board=', $context['current_board'], '.', $context['start'], ';sort=starter', $context['sort_by'] == 'starter' && $context['sort_direction'] == 'up' ? ';desc' : '', '">', $txt['started_by'], $context['sort_by'] == 'starter' ? ' <img src="' . $settings['images_url'] . '/sort_' . $context['sort_direction'] . '.gif" alt="" />' : '', '</a></div></th>
					<th scope="col" width="14%"><div class="lff-topic-list-header-content"><a href="', $scripturl, '?board=', $context['current_board'], '.', $context['start'], ';sort=replies', $context['sort_by'] == 'replies' && $context['sort_direction'] == 'up' ? ';desc' : '', '">', $txt['replies'], $context['sort_by'] == 'replies' ? ' <img src="' . $settings['images_url'] . '/sort_' . $context['sort_direction'] . '.gif" alt="" />' : '', '</a> / <a href="', $scripturl, '?board=', $context['current_board'], '.', $context['start'], ';sort=views', $context['sort_by'] == 'views' && $context['sort_direction'] == 'up' ? ';desc' : '', '">', $txt['views'], $context['sort_by'] == 'views' ? ' <img src="' . $settings['images_url'] . '/sort_' . $context['sort_direction'] . '.gif" alt="" />' : '', '</a></div></th>';
			// Show a "select all" box for quick moderation?
			if (empty($context['can_quick_mod']))
				echo '
					<th scope="col" class="lefttext last_th" width="22%"><div class="lff-topic-list-header-content"><a href="', $scripturl, '?board=', $context['current_board'], '.', $context['start'], ';sort=last_post', $context['sort_by'] == 'last_post' && $context['sort_direction'] == 'up' ? ';desc' : '', '">', $txt['last_post'], $context['sort_by'] == 'last_post' ? ' <img src="' . $settings['images_url'] . '/sort_' . $context['sort_direction'] . '.gif" alt="" />' : '', '</a></div></th>';
			else
				echo '
					<th scope="col" class="lefttext" width="22%"><div class="lff-topic-list-header-content"><a href="', $scripturl, '?board=', $context['current_board'], '.', $context['start'], ';sort=last_post', $context['sort_by'] == 'last_post' && $context['sort_direction'] == 'up' ? ';desc' : '', '">', $txt['last_post'], $context['sort_by'] == 'last_post' ? ' <img src="' . $settings['images_url'] . '/sort_' . $context['sort_direction'] . '.gif" alt="" />' : '', '</a></div></th>';

			// Show a "select all" box for quick moderation?
			if (!empty($context['can_quick_mod']) && $options['display_quick_mod'] == 1)
				echo '
					<th scope="col" class="last_th" width="24"><div class="lff-topic-list-header-content"><input type="checkbox" onclick="invertAll(this, this.form, \'topics[]\');" class="input_check" /></div></th>';

			// If it's on in "image" mode, don't show anything but the column.
			elseif (!empty($context['can_quick_mod']))
				echo '
					<th class="last_th" width="4%"><div class="lff-topic-list-header-content">&nbsp;</div></th>';
		}
		...
		foreach ($context['topics'] as $topic)
		{
			...
			if ($topic['new'] && $context['user']['is_logged'])
					echo '
							<a href="', $topic['new_href'], '" id="newicon' . $topic['first_post']['id'] . '"><img src="', $settings['lang_images_url'], '/new.png" alt="', $txt['new'], '" class="lff-new-topic-flag" /></a>';
	...
	<input type="hidden" name="' . $context['session_var'] . '" value="' . $context['session_id'] . '" />
		...
		renderListNavigation($buttons, 'bottom');
	}
